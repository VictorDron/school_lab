    ...
    <div className="border border-neutral-200 rounded-lg overflow-hidden">
      <div
        ...
        onClick={
          onNavigateToCalendar
            ? () => onNavigateToCalendar(event.id, new Date(event.startDate))
            : undefined
        }
      >
        <div
      ...
      {canEdit && isScheduled && (
        <div className="flex border-t border-neutral-100">
          <button
            onClick={() =>
              updateStatusMutation.mutate({ id: event.id, leadId: event.leadId, status: 'COMPLETED' })
            }
          ...
          <button
            onClick={() =>
              updateStatusMutation.mutate({ id: event.id, leadId: event.leadId, status: 'CANCELLED' })
            }
            disabled={updateStatusMutation.isPending}
